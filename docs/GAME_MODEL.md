...
**Behaviours**, each a method, each citing its source. The sources are in `docs/UNIQUE.md` unless noted; a behaviour without a source does not go in.
...
- `docs/UNIQUE.md` stays the record of measurements and gains a line per new one; this file points there and never repeats a number.
