...
- **A bash is a right-hand attack made while blocking.** In the idle tree, `bashStart` (01B417) is a child of `NonMountedCombatRight`, the branch `ActionRightAttack` resolves through, beside `NormalAttack` (`attackStart`), under three conditions: GetWantBlocking 1, IsBlocking 1, attack state not Bash. Under `NonMountedCombatRightPower`, `PowerAttack` (0E8456, for the player not holding block with a weapon in the right hand) comes before `PowerBash` (0E8452, `bashPowerStart`), which is gated on HasPerk Power Bash **and** GetIsID Player: the tree gives a power bash to the player alone, and only while blocking. How an NPC's power bash comes about (the race data offers `bashPowerStart` at 0.33) was not read.
...
- Weapon Drawn and IgnoreCombat; the `GetIsReference` lease condition every record of ours has.

The Int, Bool and Location layouts are found at load on authored values, each against a second record: Edorfin's trigger radius 350 against the Sovngarde heroes' 1500 and their three attacks per barrage; the heroes' pause on against Edorfin's off; Edorfin's editor location of radius 32 against the heroes' alias location of radius 425 and Edorfin's near-self search location. Vilkas's training package (0F7952) confirms Target to Attack reads a specific reference as Mercer's Target does. The copy's location must be its own before it is written, and its target is written and read back. A check that fails leaves Power Attack to the animation event; the casts are not affected.

A request aims Target to Attack at the rule's enemy and leases the record as a cast is leased. The procedure completes after its attack, but its condition still passes then and the AI would pick it again, so the lease ends as soon as the swing does: a power attack seen in the follower's attack data after the AI picked the package up, then the attack state back at none (`power attack made`). A power attack already running when the lease began is not taken for ours. Otherwise the lease ends when the AI drops the package, or at a deadline: 3 s to pick up and strike, 3 s more once the swing is seen.
...
**Bash and Power Bash** (`src/game/Blows.cpp`). A request is steps: the weapon drawn and no swing in progress; the block raised with `CombatAnimation::Execute(actor, 3)` unless it is up already; `bashStart` or `bashPowerStart` sent to the graph once `IsBlocking`, again on each step while the graph refuses it; the bash attack state watched until it ends (`bash made`), or 1.5 s; the block lowered with 4 if the request raised it. A request not done 2 s after it was made is given up, and says at which step.
...
While either is in flight the follower's other rules wait, as they do for a cast. Both report through `rule.resolved` (`docs/EVENTS.md`), with the timings and refusals that say which step held a bash up.
...
- **A power bash from a follower's block.** The tree offers one to the player alone; whether the graph takes the event from an NPC's block.
