...
- **Any buff food, in play.** Built 2026-09-16 (`docs/ACTIONS.md` "Any: the rolled choices"); not yet run. Give a follower Beef Stew, Elsweyr Fondue and a stack of apples; `IF self: any THEN self: Food -> Any buff` should eat the stew and the fondue once each and never an apple, then read "every buff carried is already up" for twelve minutes. Watch `Buffs` against a load order's own foods, and whether the roll's spread looks even over a long fight.
- **Does a food's effect stack with a potion's of the same name?** `docs/ACTIONS.md` "What stacks" compares effects by NAME, and a food's record is not a potion's -- Elsweyr Fondue's Fortify Magicka is `FoodFortifyMagicka` (1058A8), a potion's is `AlchFortifyMagicka` (03EAF8), and the same holds for Fortify Stamina and Regenerate Magicka. So a stew's Fortify Stamina in force keeps a follower off a weaker Fortify Stamina potion. Eat Elsweyr Fondue, drink a weaker Fortify Magicka potion, and read the follower's magicka maximum: if it rises, the engine keeps strongest-only per RECORD, the two stack, and `PotionStock::Outdone` should compare the record rather than the name.
...
- **Power attack and bash, in play** (`docs/ATTACK.md` "What was built", 2026-09-15, not yet run). At load, the debug line `probe: UseWeapon inputs calibrated`, or the warning that Power Attack goes by the animation event, and `probe: alias override lists readable`, or the warning saying which part of the check failed; the follower's `packages:` line names their power attack package. In a fight, `rule.resolved` for `power attack`, `bash` and `power bash`: `made` against `not-made`; for a power attack, that the lease ends on `power attack made` and the follower does not go on to a second one; for a bash, `bashS` against `staminaAtRequest` and `staminaAtEnd`: the engine's own bash holds the bash state about half a second and costs `fStaminaBashBase` 35, where the two shortest made on 2026-09-15 held it 0.09 s. A power bash is measured and working (`docs/ATTACK.md`); a plain bash has not been read the same way. A sword and shield follower (Bash, Power Attack) and a follower with a spell in the right hand and a dagger in the left (Power Attack by the event) cover the routes. In a fight, a power attack's `placedIn` should read `combat override list` (Karliah's package data, no turning; `docs/ATTACK.md` "The override list route"), and out of one `alias packages`; after a load, the follower alias's list should hold none of our records.
- **The Settings requirements, in play** (built 2026-09-15, `docs/PROFILES.md` "The settings record"). Three switches under Customize: dual wield combat style (on), dual casting perks (off), power bash perk (off). Verify each way round: with the perk requirements off, a follower with no perk is offered Dual Cast and Power Bash and both fire; with them on, neither is offered and a written rule for either reads as unavailable, the log saying `no-perk` or `cannot-dual-cast`. With the dual wield requirement off, a follower whose style forbids it keeps a weapon in each hand; turning it back on takes nothing off by itself (the AI decides again). Then save, reload, and check the three read back as they were left, along with the switch over all followers, and that a save made before the record existed loads the defaults (everything on but the two perk requirements). A follower's own switch was already saved in their own record; verify it still is.
