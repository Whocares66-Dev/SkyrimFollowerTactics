...
- **The follower's own spell numbers, in play.** 2026-09-05: the Magic tab's magnitude, duration, effect lines and description, and the Reanimate cap, are the follower's -- the record's number through the ModSpellMagnitude / ModSpellDuration perk entry points, which is how the engine makes the effect and how the Fortify actor values reach it (`docs/RESEARCH.md` 6). Cost was already theirs. To confirm: a follower with a Fortify Destruction robe or Augmented Flames shows a higher Firebolt than the record's 25, and the description agrees with the effect line. The description is asked for with no parent so its tokens are left for us to fill; if it comes back already filled (the engine's, for the player), the two would differ and the parent-less call is not what it looked like.
