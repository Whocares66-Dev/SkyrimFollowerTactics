...
## 9a. Not: the condition negated (built 2026-09-17, not yet seen in play)

A **Not** column sits between the rule's number and its condition, a tick like the On switch: ticked, the rule holds when its condition does NOT. "No enemy is undead", "the player is not sneaking", "this weapon is not poisoned" are each one rule now, where before they needed a predicate of their own -- which is why the pairs that exist (Summon none / active, Weapon poison none / active) exist at all. The tooltip says *Click to negate condition* and, once ticked, *Click to unnegate condition*. On the wire it is `"not": true` inside the rule's `if`, written only when it is on.

**A negated condition matches nobody.** For a group subject, Not means "not one of them": `NOT Enemy: Undead` holds while no enemy is undead, so there is no enemy it matched, and the action targets that mean *the one the condition matched* -- Ally, Corpse -- are not offered for it and are put back to Self by `Reconcile` when the tick goes on. Enemy and Attacker survive negation, because neither needs the binding: Enemy is then whoever the follower is fighting (else the nearest), and Attacker is whoever last hit them, exactly as under a Self condition. The rule's subject, for the log and for the action, is the follower themself.

**Three conditions cannot be negated** (`CanNegate`), and their cell is dead with a tooltip saying so: **Any**, which is the always-true condition and negates to a rule that can never fire, and **Combat begins** / **Combat ends**, which are moments -- "not the tick the fight began" is every other tick of it, which is not what anyone means by Not. A profile hand-edited to carry one keeps the rule and drops the flag.

**The two guards around a condition are outside the negation**, and this is the part worth not getting wrong: a subject/predicate pair that cannot be asked at all stays unanswerable rather than becoming true, and the farewell evaluation after a fight still runs only Combat end rules. Negating inside those guards would fire a Not rule on the tick after a fight, and on every pair the editor refuses -- the two places its plain twin is never even evaluated. Covered by the tests in `tests/test_conditions.cpp` (`[not]`).
