...
Screenshots live in `assets/img/panel/`. `settings.png` shows the full Settings page; `settings_requirements.png` shows perk and dual-wield requirements; `settings_combat_ai.png` shows the two AI options. The new training screenshots appear in `guide/training.md`.
...
Pages with `math: true` in their front matter load MathJax; write TeX between `$$` delimiters, inline or on lines of their own.
...
- The site root shows the current `master` guide, labeled by `docs_version` in `_config.yml` (currently `0.4.0`). Local previews use the working tree.
