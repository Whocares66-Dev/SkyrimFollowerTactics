...
| `ActorEquipManager::UnequipSpell(actor, spell, source)`, what Papyrus's `Actor.UnequipSpell` tail-calls after its null check: source 0 the left hand, 1 the right, 2 the voice, turned into the matching equip slot inside | 38903 | `UnequipSpellNow`, `src/game/Pins.cpp` | the Papyrus dispatch instead, which the VM runs a frame later; the panel's cell answers on its next frame (`RefreshShownPageSoon`) rather than at once |
