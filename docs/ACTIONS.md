...
**Bash and Power Bash (built 2026-09-09, not yet seen in play).** The race data's other two blows: `bashStart`, flagged Bash Attack, and `bashPowerStart`, flagged Bash Attack and Power Attack, x1.5 damage and the hardest stagger of any entry. What blocks is what bashes, by the vanilla block rule: a shield or a torch in the left hand, else any weapon in the right hand with the left hand empty (a one-hander, a two-hander, a bow, a crossbow, a staff). A weapon alone in the left hand cannot block, so a left-hand staff does not bash; nor do two hands each holding something (a blade and a staff, a blade and a spell), the fists, or a spell hand ("no weapon"). The cost is the setting for the kind, `fStaminaBashBase` 35 and `fStaminaPowerBashBase` 55, times the entry's multiplier (1 for both); no perk entry point prices a bash. The reach is `fCombatBashReach` (141) at the actor's scale with the same body margin as a swing. The player needs the Block perk Power Bash to power bash; that perk has no entries, so the gate is in the input handling, and an NPC's power bash through the graph is not gated by it -- the AI's own picks it with a chance of 0.33 in the race data. The three blows share the snapshot's `Blow` (possible, stamina, reach), the evaluator's checks, and Execute.
