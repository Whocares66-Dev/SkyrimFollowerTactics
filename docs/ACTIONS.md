...
That is the lever if a quarter is not enough: dispatching a blow is cheap and refusal is free, so the cooldown could be spent on the *result* rather than on the decision, letting a refused blow retry on the very next tick. That means the game side reporting back into the evaluation context, which nothing does today, so it is a design change and not a tweak. Not made yet, and the honest reason to wait is that a follower who power-attacks every 1.5 s whenever the graph allows may simply look frantic; the number to watch first is whether three in eleven is a problem in play or just what a follower's own attack rhythm looks like.
