...
**Bash and Power Bash (built 2026-09-09; made from a raised block since 2026-09-15, `docs/ATTACK.md`).** The race data's other two blows: `bashStart`, flagged Bash Attack, and `bashPowerStart`, flagged Bash Attack and Power Attack, x1.5 damage and the hardest stagger of any entry. What blocks is what bashes, by the vanilla block rule: a shield or a torch in the left hand, else any weapon in the right hand with the left hand empty (a one-hander, a two-hander, a bow, a crossbow, a staff). A weapon alone in the left hand cannot block, so a left-hand staff does not bash; nor do two hands each holding something (a blade and a staff, a blade and a spell), the fists, or a spell hand ("no weapon"). The cost is the setting for the kind, `fStaminaBashBase` 35 and `fStaminaPowerBashBase` 55, times the entry's multiplier (1 for both); no perk entry point prices a bash, as the engine's own routine confirms (26429). The reach is `fCombatBashReach` (141) at the actor's scale with the same body margin as a swing. The player needs the Block perk Power Bash (058F67; Shield Wall and Block 30 to take it) to power bash. The perk has no entries: the gate is the idle tree, where `PowerBash` (0E8452, `bashPowerStart`, under the right power attack branch) requires HasPerk Power Bash **and** GetIsID Player (read 2026-09-15), so the tree never offers it to an NPC, perk or not. Only four NPC records carry the perk anyway (the Ebony Warrior and three Creation Club Hands). An NPC's power bash sent straight to the graph is not gated by either -- two landed in play on 2026-09-09 -- and the race data gives it a chance of 0.33; how the combat AI's own power bashes reach the graph past the tree was not read. Falmer have a `bashPowerStart` idle of their own under their left attack branch, with no perk. The three blows share the snapshot's `Blow` (possible, stamina, reach), the evaluator's checks, and Execute.
