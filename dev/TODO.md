...
- **Split `UI.cpp` (rule editor, sheet pages, widgets), and move the spell math out of `Inventory.h`.** Mechanical but large, and only verifiable in play; the shared pieces both halves would need are in `game/Sheet.h` now, so the split is a move. `Sensors.cpp` was split this way on 2026-09-25, by family: the snapshot stays, beside `Bag`, `Effects`, `Spells`, `Traits`, `Values`, `EffectRows`, `CharacterSheet` and `PerkSheet`, the blow planning in `Blows` and `GameSetting` in `Util`; every definition moved as it was, a few file-local helpers made shared under a header.
