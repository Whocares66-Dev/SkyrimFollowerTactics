...
- **Out of combat, as a second list.** Buff upkeep out of a fight is the point of the player's tactics and is not built: it is to be a list of its own beside the combat one -- a tab, or a section that folds -- evaluated out of a fight as the combat list is in one, not a per-rule "when" (decided 2026-09-18). Needs the tick to build a snapshot out of combat for an actor with such a list, and the player's holds (`PlayerHeld`) already gate it.
...
- **Power attack and bash, in play** (`dev/ATTACK.md` "What was built", 2026-09-15, not yet run). At load, the debug line `probe: UseWeapon inputs calibrated`, or the warning that Power Attack goes by the animation event, and `probe: alias override lists readable`, or the warning saying which part of the check failed; the follower's `packages:` line names their power attack package. In a fight, `rule.resolved` for `power attack`, `bash` and `power bash`: `made` against `not-made`; for a power attack, that the lease ends on `power attack made` and the follower does not go on to a second one; for a bash, `bashS` against `staminaAtRequest` and `staminaAtEnd`: the engine's own bash holds the bash state about half a second and costs `fStaminaBashBase` 35, where the two shortest made on 2026-09-15 held it 0.09 s. A power bash is measured and working (`dev/ATTACK.md`); a plain bash has not been read the same way. A sword and shield follower (Bash, Power Attack) and a follower with a spell in the right hand and a dagger in the left (Power Attack by the event) cover the routes. In a fight, a power attack's `placedIn` should read `combat override list` (Karliah's package data, no turning; `dev/ATTACK.md` "The override list route"), and out of one `alias packages`; after a load, the follower alias's list should hold none of our records.
