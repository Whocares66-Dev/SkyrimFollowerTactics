...
**What we do instead.** Bind both by ID and call them on the game thread (`UnequipSpellNow` and `UnequipShoutNow`, `src/game/Pins.cpp`). There is no Papyrus fallback: the plugin is built for the AE line alone since 2026-09-17, so the IDs always resolve. The IDs were read on 1.6.1170 but are not pinned to it: both are in the 1.6.317, 353, 629, 1130, 1170 and 1179 databases, each at its own offset, so they hold across the AE line. The guard is `REL::Module::IsAE()` and it is about SE and VR, whose Address Library is a separately numbered ID space -- the same number there names another function, which is why CommonLib pairs every such ID as `RELOCATION_ID(se, ae)`. The rows are in `dev/VERSIONS.md`. Not yet verified in play.

**Upstream.** A PR would add the two to `ActorEquipManager.h` and `.cpp` beside `EquipSpell` and `EquipShout`, as `RELOCATION_ID(se, ae)` pairs. The AE halves are 38903 and 38904, read here; the SE halves still need reading against a 1.5.97 binary before either could be offered, which is the same work `dev/VERSIONS.md` already owes for these IDs. Not reported yet, and whether an issue exists was not checked -- `gh` is not installed here.
