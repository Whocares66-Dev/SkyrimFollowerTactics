...
| `player.held` / `player.free` | the player's own evaluation is held -- in dialogue, the fighting controls off, in furniture, mounted, knocked down, swimming, in a kill move, in beast form -- and free again | `reason` (held) |
...
| `rule.resolved` | a requested cast, scroll, shout, power, power attack or bash is over | `ruleIndex`, `ruleName`, `kind`, `outcome` (`cast` or `not-cast`; `made` or `not-made` for a power attack or a bash), `reason`, `durationS`. A cast, scroll, shout or power adds the form, the target's ids, `pickedUp` and `placedIn` (`alias packages`, or `nowhere`). A power attack adds `attackEvent` (the event the action carried), `waited` (a step could not take it at once), `sentS` and `hitS` (seconds after the request that the action was taken and that it reached its hit, -1 for never), `refusals` (the action turned away), the follower's `staminaAtRequest` and `staminaAtEnd`, and `offStrikeAtRequest` and `offStrikeAtEnd` (degrees between the follower's heading, less the attack's angle, and the target; -1 where there is no one to measure). A bash or power bash adds `alreadyBlocking`, `blockRaised`, `blockUpS` and `sentS` (seconds after the request, -1 for never), `blockRefusals` and `bashRefusals`, `waited` (a step could not go on at once, so the bash settled first), `handsFreeS` and `steadyS` (seconds after the request that the hands were last found free, and free with the block up; -1 for never), `attackStateSeen` (the first attack state other than a bash once the bash was taken, -1 for none), `bashS` (how long the bash attack state lasted, -1 for never), `attackEvent` (the current attack data's event when it began: `bashStart` or `bashPowerStart`), and the follower's `staminaAtRequest` and `staminaAtEnd` (`dev/ATTACK.md`). A cast on the player's own body (`kind` `cast` or `voice`; module `player`, `dev/PLAYER.md`) adds the form, `hand` (`left`, `right`, `both`, `voice`), `lent` (the hand held something else, put back after), `drew` (the hands were sheathed and drawn for it), `pressedS`, `readyS`, `releasedS` and `firedS` (seconds after the request, -1 for never), `highestState` (the caster's state as far as it got: 3 is Ready, 6 Casting), for a shout `wordsWanted` (the highest word unlocked, 0 for the first alone) and `wordsHeld` (the level the control was released at), and `magickaAtRequest` and `magickaAtEnd` |
...
**`reason`** on `rule.resolved` is why the request ended. A cast's, scroll's, shout's or power's is the package's own release reason: `spell fired`, `shout fired`, `power fired`, `stream ended`, `target dead`, `package ended`, `deadline, never cast`, `deadline, shout never ended` (a shout or power picked up that never fired), `deadline, AI never picked it up`, `deadline, stream still running`, `holder vanished`, `saving`. A power attack's: `power attack made`, `power attack cut short`, `taken, never swung`, `watch over, still swinging`, `watch over, hit but still swinging`, `deadline, weapon never drawn`, `deadline, still mid-swing`, `deadline, still mid-cast`, `deadline, target never in front`, `deadline, refused`, `holder vanished`, `saving`. A bash's: `bash made`, `taken, never bashed`, `taken, an attack but no bash`, `watch over, still bashing`, `deadline, weapon never drawn`, `deadline, still mid-swing`, `deadline, block refused`, `deadline, block never up`, `deadline, bash refused from the block`, `deadline, never steady`, `holder vanished`, `no actor state`, `saving`. A cast on the player's: `spell fired`, `stream ended`, `shout fired`, `power cast`, `refused` (the engine's CheckCast said no; the prose names its reason), `the hand would not take the spell`, `the voice would not take it`, `deadline, hands never drawn`, `deadline, hands never free`, `deadline, never ready`, `the charge was cut short`, `the stream was cut short`, `stream ended early`, `released, ended without firing`, `released, never fired`, `player vanished`, `no actor state`, `saving`.
