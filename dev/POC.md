...
| The panel | `src/progression/game/UI.cpp` | Written; never seen |
...
| File | Does |
|---|---|
| `Actors.cpp` | Who is a follower (teammate, person, not dismissed, alive), waiting, unique; skills and attributes as the engine has them; assigned points into the permanent modifiers; the perks on a record (read only) |
...
| `Service.cpp` | The ledger's home: the paced tick (party, enrolment, the rules re-read, levels, reconciling what they have), each skill use, every action from the panel, the co-save's contents |
...
| `UI.cpp` | The panel, under Tactics' section: Overview, Settings, one entry per companion with Skills, Perks and Spells |
...
| A bought ability perk (Recovery, Magic Resistance) is an `AddSpell`, and the save keeps it: "nothing is written to the save" was wrong | Said so; turning levelling off (Tactics' Settings) takes everything of ours off each companion, abilities included, keeping the ledger for turning it on again |
...
1. **It loads.** The log says the perk trees were read (`read 180 perks (251 ranks)` on vanilla; more with mods) and that the panel registered. F1 shows *Follower Tactics* with *Progression*, and Overview and Settings under it; Tactics' Settings has *Enable leveling for followers*, on.
2. **The trees are the vanilla trees.** Settings → *Write the perk trees to the log folder*; compare `FollowerTactics.perks.json` with `tests\progression\data\vanilla-perks.json`. Verdicts and requirements should match node for node on an unmodded order.
3. **Enrolment.** Recruit Lydia (or any unique follower). Within a second they appear on the Overview, at their engine level, and get their own entry under their own name. Settings → *Learning* lists the rules as the game has them: `fSkillUseCurve` 1.95, a level 75 + 25 × level.
4. **Learning by doing, and the modifier.** Select them in the console, `bat fpcheck`, note the permanent modifiers (all 0). Fight something with them (Tactics' `bat ftspawn` places a draugr). Settings: the uses heard move for blows landed and hits taken; with a caster companion, from magic too. Skills tab: the *Next level* bars have moved, and after enough, `One-Handed` rises by one — `bat fpcheck` shows the permanent modifier at 1, and the log says *increased to*.
5. **A perk, and its effect.** First Settings → *Check perks against the engine*: the log line for each companion present should read *N of N perks as the view has them*, and the hook counters under the button should be moving. Then Perks tab → Armsman → Learn → Confirm; `hasperk 000BABE4` is 1 (the console's `hasperk` is the engine's `HasPerk`, which is the hooked `ForEachPerk`). The DESIGN.md P0 is the *effect*: compare their one-handed damage before and after (Follower Tactics' Character tab builds the hands' damage from `ForEachPerkEntry`, each Mod Attack Damage entry a line). Unlearn it: damage goes back, `hasperk` is 0. Set aside one of Marcurio's own perks and restore it the same way.
6. **Save, load, restart.** Save; load it; `bat fpcheck`: the modifiers are unchanged (not doubled — the ledger's *applied* came back with the co-save), `hasperk` still answers from the view, and *Check perks* agrees. Quit to desktop, relaunch, load: the same.
7. **Save A / save B.** Make save B *before* step 3 and save A after step 5. Load A, then B: in B, `hasperk 000BABE4` on Lydia must be 0. With the view this should hold by construction (nothing is written to the record); this step is where that is confirmed. Then, on a copy of save A, buy them Recovery first if a Restoration companion is to hand (an ability perk), and turn levelling off (Tactics' Settings): *Check perks* reads their record's perks, `bat fpcheck` shows the modifiers at 0, `hasspell` on Recovery's ability is 0. Save, untick the mod, load: the same, and nothing in the log about missing records. Turning levelling on again, on the save made before unticking, puts it all back.
8. **Levels and points.** Settings → *Give 10,000 experience*: the level rises (at most 5 past yours), *Lydia reached level N: … to assign*, and the Skills and Perks tabs have the points. Assign an attribute: `bat fpcheck` shows +10. A follower the engine caps (Onmund at 30) with you past 30: their level climbs above 30 only by what they learn.
9. **Teaching.** `bat fpsetup`, then their Spells tab: Flames can be taught, Fireball says *Needs Destruction 50*. Teach Flames: one tome goes, `hasspell 00012FCD` is 1 (the console's `hasspell` is the engine's `HasSpell`, which walks the detoured `VisitSpells`), and *Check perks and spells* agrees. The P0 here is the **cast**: Lydia, taught Flames and nothing else, should use it in the next fight. It is known only through the view, never added to them. Forget it: the tome does not come back, `hasspell` is 0. Then Marcurio: set aside Sparks, and `hasspell 0002B96B` on them is 0 and they don't cast it in a fight; restore it and both come back.
10. **Moving skills.** With Armsman learned, `-` on One-Handed down to its requirement: it greys there and its hover names Armsman. *Reset* One-Handed: it goes to 15 plus the race's bonus, Armsman is returned, and the pool shows what it was worth; `+` on Two-Handed spends it, a level at a time. `bat fpcheck` follows each click.
...
- **The permanent modifier.** Assumed to persist in the save for an NPC and to survive vanilla level-up recalculation (Tactics' MODIFIERS.md: on an NPC, effects write the temporary modifier, so the permanent one is otherwise quiet). If it does not persist, step 6 shows the modifier at 0 after a load with the ledger believing it applied; the reconcile would need a read-back (the modifier as the engine has it against what we applied), which is sketched in PROGRESSION.md's engine table.
...
- **Ability perks.** A perk that works by adding an ability (Magic Resistance, Recovery) is an `AddSpell` on the actor, and the save keeps it: uninstalling without turning levelling off first leaves a bought ability behind. Step 7 checks that releasing takes it off.
