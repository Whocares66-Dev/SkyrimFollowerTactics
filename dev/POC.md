...
Tactics' build, and its checks (CLAUDE.md, "Before every commit" and "Before a merge or a push"): Progression's core is `fp_core`, its tests `fp_tests`, both run by the `core` presets' tests; the formatter and both tidy targets cover `src/progression`.
...
| A bought ability perk (Recovery, Magic Resistance) is an `AddSpell`, and the save keeps the temporary modifier it makes: "nothing is written to the save" was wrong | `BeforeSave` takes bought abilities off before each write and puts them back after; **seen 2026-09-22**, the follower reads their own value with the mod unticked, no step first |
...
- **Attribute points from their own values.** An NPC's health, magicka and stamina come from their class, and count as points spent: a follower their class carried past the player's count (Serana at 50) has none from their level, and moves what they have instead, down to their race's starting values.
...
- **Ability perks.** A perk that works by adding an ability (Magic Resistance, Recovery) is the engine's `AddSpell`, and what the ability does to a value is a temporary modifier the save keeps -- measured 2026-09-22 in the test setup: a follower's Magic Resistance still +10 with the plugin gone, the spell listed nowhere and nothing left to dispel. So a bought perk's abilities come off before every save and go back on after it (`BeforeSave`), which leaves nothing in the file. **Confirmed in play 2026-09-22**: with that build, a save made with Magic Resistance bought, loaded with the plugin unticked, reads the follower's own Magic Resistance. The SKSE task does run after the write, which the ordering of SKSE's `kSaveGame` had only implied.
