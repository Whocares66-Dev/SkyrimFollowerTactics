...
1. **It loads.** The log says the perk trees were read (`read 180 perks (251 ranks)` on vanilla; more with mods). Tactics' Settings has *Enable leveling for followers*, on.
...
4. **Learning by doing, and the modifier.** Select them in the console, `bat fpcheck`, note the permanent modifiers (all 0). Fight something with them (Tactics' `bat ftspawn` places a draugr). After enough, `One-Handed` rises by one on their Skills tab — `bat fpcheck` shows the permanent modifier at 1, and the log says *increased to*; with a caster companion, from magic too.
...
6. **Save, load, restart.** Save; load it; `bat fpcheck`: the modifiers are unchanged (not doubled — the ledger's *applied* came back with the co-save), `hasperk` still answers from the view, and the skill page draws the same. Quit to desktop, relaunch, load: the same.
7. **Save A / save B.** Make save B *before* step 3 and save A after step 5. Load A, then B: in B, `hasperk 000BABE4` on Lydia must be 0. With the view this should hold by construction (nothing is written to the record); this step is where that is confirmed. Then, on a copy of save A, buy them Recovery first if a Restoration companion is to hand (an ability perk), and turn levelling off (Tactics' Settings): the skill pages show their record's perks, `bat fpcheck` shows the modifiers at 0, `hasspell` on Recovery's ability is 0. Save, untick the mod, load: the same, and nothing in the log about missing records. Turning levelling on again, on the save made before unticking, puts it all back.
8. **Levels.** As they learn, the level rises (at most 5 past yours) and the log and a notification say *Lydia reached level N: … to assign*; the skill page's header has the perks to spend. A follower the engine caps (Onmund at 30) with you past 30: their level climbs above 30 only by what they learn. Attributes have no page yet.
...
10. **Teaching.** From a tome in the follower's inventory, with a Forget on a spell's page: being built on `wip-spell-teach`.
