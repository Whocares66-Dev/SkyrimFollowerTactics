...
def main():
    ...
    for name, path in PLUGINS.items():
        ...
        for kind, form, fields in rows[1:]:
            edid = next((v.rstrip(b'\0').decode('utf-8', 'replace') for k, v in fields if k == b'EDID'), '')
            source = identity[form >> 24]
            details.append(dict(type=kind, form=f'{form & 0xffffff:06X}:{source}', editorid=edid,
                                override=source != path.name))
        (ROOT / (name + '-records.json')).write_text(json.dumps(details, indent=2), encoding='utf-8')
        summary[name] = dict(masters=masters, records=len(details),
    ...
    (ROOT / 'plugin-summary.json').write_text(json.dumps(summary, indent=2), encoding='utf-8')
