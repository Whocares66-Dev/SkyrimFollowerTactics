...
REQUESTS = [
    ...
    (
        "flames-tome",
        "housecarl_read_record",
        {"formid": "09CD51:Skyrim.esm", "depth": 3, "resolve_names": True},
    ),
    (
        "warrior-class",
        "housecarl_read_record",
        {"formid": "013176:Skyrim.esm", "depth": 3},
    ),
    (
        "flames",
        "housecarl_read_record",
        {"formid": "012FCD:Skyrim.esm", "depth": 3, "resolve_names": True},
    ),
    (
        "armsman",
        "housecarl_read_record",
        {"formid": "0BABE4:Skyrim.esm", "depth": 4, "resolve_names": True},
    ),
]
...
def main():
    env = dict(
        os.environ,
        HouseCarl__Mo2InstanceDir=r"C:\modding\MO2",
        HOUSECARL_DATA_DIR=str(ROOT / "housecarl-cache"),
    )
    proc = subprocess.Popen(
        [r"C:\modding\houseCARL\housecarl\server\housecarl-mcp.exe"],
    ...
    def call(i, method, params):
        proc.stdin.write(
            json.dumps(dict(jsonrpc="2.0", id=i, method=method, params=params)) + "\n"
        )
        proc.stdin.flush()
        while True:
