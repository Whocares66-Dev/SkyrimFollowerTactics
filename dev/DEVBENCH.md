...
`dev/TESTING.md` is built around the limits of `bat` files: their lines run in no reliable order, `prid` cannot be batched, a read cannot bracket a write, and a `placeatme` copy has to be clicked. Over devbench:
