...
| shout package | a copy of `MQ305TsunReturnShout` (Shout template), Shout = its wrapper, Target = Self, HoldWhenBlocked off, flags IgnoreCombat and WeaponDrawn. Same condition. How a POWER is performed (`dev/ACTIONS.md` 7) |
...
The game's own example of a package cast mid-fight is Mercer Frey in *Blindsighted*: a UseMagic package gated on quest stage makes him cast Nightingale Strife at the player. Ours is the same package with `GetIsReference` as the trigger, on the actor's stack. It is the rare use: of the 78 packages on the `UseMagic` and `UseMagicRepeat` templates across the load order (2026-09-15), nearly all are staged casts (College practice, Ancano at the Eye, Neloth), and only a handful override combat -- Mercer's three, `SprigganCallOverride`, `dunShimmermistFalmerCastPackage`, `DLC1dunHarkonCastInvulnerabilityShield`. The combat AI's own casts never run the procedure; they come from its `CombatMagicCaster` classes ("What the combat AI will cast" below).
...
2. **The AI casts.** Our package now passes. `IgnoreCombat` takes her hands
   ...
   doing, charges, fires. Animation and interruption are the game's; the
   cost is the game's only because we refuse the procedure's skip of it
   ("What a cast costs" below).
...
**Until 2026-09-25 a cast through our record cost nothing** (issue #2): the UseMagic procedure has the caster skip its checks, and the cost goes with them. Read from 1.6.1170, and the procedure's calls on 1.5.97 too:

- **Where a cast pays.** Most spells pay in the actor caster's `Update` (34143): bit by bit over the charge, into `costCharged`, and each second of a concentration stream, cutting the cast off when the actor can no longer pay. `SpellCast` (34144) pays at release only for a fire-and-forget spell with no charge time. `CheckCast` (34145) refuses a cast the actor cannot afford. What is paid is the actor value 34609 names: magicka for a spell, a lesser power or a poison; the weapon's charge for an enchantment or a staff; nothing for a scroll or a power.
- **The skip.** `ActorMagicCaster::SetSkipCheckCast`, vtable slot 0x13 (34160; 33379 on SE), sets flag 2 at `+0xF8`. With it set, `CheckCast` returns "can cast" before any test, and `Update` passes over its whole cost block. `FinishCastImpl` and `InterruptCastImpl` clear it. `SpellCast` does not read it.
- **Who sets it.** The UseMagic procedure's cast step (29346; 28593 on SE) calls slot 0x13 just before it starts a one-hand cast, and on every update while its caster is charging, ready or casting. None of its eleven inputs (Location, Spell, Target, HoldWhenBlocked, CastTime and Cooldown min and max, NumToCast min and max, DualCast) governs it. A dual cast starts by another path (38762, which calls the same start as one hand) that does not set it, and the per-update call sets it all the same. Made for staged casts, which should not run dry mid-scene. The player's casts come through the input handlers and the combat AI's through its own casters, and neither sets it.
- **What we do.** `ChargeOurCasts` (`src/game/Packages.cpp`) replaces slot 0x13 in `ActorMagicCaster`'s vtable. When the caster's actor holds their spell record's lease and the caster's current spell is that record's spell, or empty, the flag is cleared instead of set; every other call goes to the original. The current spell (`MagicCaster::currentSpell`) is what the caster is casting, not what the hand has equipped (`Actor::selectedSpells`): the start of a cast (34401) refuses a caster whose current spell is set, and otherwise sets it (`SetCurrentSpell`, 34422), and the engine empties it as each cast fires ("Facts worth not rediscovering"). The procedure's first call comes just before that start, so the current spell is empty there whenever a cast follows; once a cast is under way it is what `Update` charges for. Each follower's casters answer for that follower's own record, so followers casting at once each pay for their own spell, from their own magicka. Our powers and shouts (the voice slot, the Shout procedure) and every other actor's casts are untouched.
- **What it brings with the cost.** The rest of `CheckCast`, which the follower's own casts meet already: no cast while shouting, no ranged spell underwater, an effect's own start test. A cast the engine refuses leaves the procedure in its ready state, trying again each update, until the lease's deadline; the rule engine refuses a spell the follower cannot afford before it asks (`CannotAfford`), so this should be rare.
...
Tracked in `dev/TODO.md`. The casting items there: sustain length in the
