...
- **What we do.** `ChargeOurCasts` (`src/game/Packages.cpp`) replaces slot 0x13 in `ActorMagicCaster`'s vtable. When the caster's actor holds their spell record's lease and the hand holds that record's spell, or no spell yet (the call before the start comes first, and a hand's spell is cleared as each cast fires), the flag is cleared instead of set; every other call goes to the original. Each follower's casters answer for that follower's own record, so followers casting at once each pay for their own spell, from their own magicka. Our powers and shouts (the voice slot, the Shout procedure) and every other actor's casts are untouched.
