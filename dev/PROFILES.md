...
A saved pin is taken back if, when the follower is first seen, they still **carry** the thing and it is still pinnable; otherwise it is forgotten with a `warn` line naming the variant. Carried, not worn: that is the one question a pin is ever dropped for, and the watchdog has always asked it that way, putting a pin's thing back on whenever it is found off. Until 2026-09-20 the load asked a different question -- was it worn, in those hands, right now -- on the reasoning that a pin is a promise about what is worn and a load re-dresses nobody. That forgot the pin of any follower who had merely sheathed or swapped since the save, and dismissing a follower and taking them back was enough to do it: their pins went and their bans, which never asked, stayed. The two now ask the same thing (`Pins.cpp`, `StillCarried`). A spell known and a shout unlocked are always carried. A ban is taken back on the same terms and dropped on the same terms (2026-09-20): a mark holds while there is something for it to hold about -- a row of the variant it names, any copy of the form where it names none -- and goes with the last of it. A ban on a form used to outlive every copy, on the reading that it had something to say the day another arrived; pins and bans are one instruction about one thing and now last as long as each other. Nothing is equipped by the load itself; the watchdog's first pass over a managed follower does that.
