...
| Spells by delivery, for the UseMagic procedure | 38727 | 37782 (92% alike) | Record, race, leveled. **Not the added spells** | `BGSProcedureUseMagic` (slot 0xC of its vtable, 29344 → 29346) and 40059. Each spell is kept only if `Actor::CheckCast` (Character slot 0x110) allows it. The procedure asks it only when the package's *Spell* input names a kind of spell rather than one (below) |
...
- **Withdrawn, until the fight ends.** A taught spell that is forgotten, or released by turning progression off, is no longer handed over by `VisitSpells`. A combat inventory gathered before still lists it, though. Until the companion is out of combat, `CheckCast` refuses it, and the tick keeps it out of their hands and out of the package's choice.
...
- **Not on VR,** which the plugin is not built for. Without the hooks -- on VR, or a detour that failed -- nothing is taught or forgotten: there is no `AddSpell` fallback, since what `AddSpell` gives stays in the save when the mod goes.
...
- **A UseMagic package that asks for a kind of spell never picks a taught one.** The procedure (29346 on 1.6.1170, read 2026-09-22) resolves the package's *Spell* input first. When it names a spell, that spell is kept (`+0x38` of the procedure's run data) and cast, and 38727 is never called: the actor's lists are not consulted. Only when the input names a kind (a type code 0x15-0x18 at `+0x40`, the `0x48822A` branch) does it gather candidates through 38727, which reads the record, race and leveled lists, but neither the added list nor the view. So a package of that kind -- a scene's, or a mod's "heal out of combat" -- passes over a taught spell. Vanilla treats a spell added with `AddSpell` the same way, so this isn't a regression. Vanilla followers run no such package.
- **Follower Tactics' pins.** Tactics treats a spell `HasSpell` denies as gone (2026-09-21): a pin or ban on it is dropped, as one on an item no longer carried (`StillCarried`, `src/game/Pins.cpp`), rather than equipped by the pin and taken off by this tick once a second. Its rules read spells through `VisitSpells`, so one set aside or forgotten reads as not known and is never named. A rule's cast is a UseMagic package whose *Spell* input names the spell (`SetPackageSpell`, `src/game/Packages.cpp`), so the procedure casts it without gathering: a taught spell is cast by a rule as any other.
