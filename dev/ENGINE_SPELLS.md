...
- **That it runs.** Settings → *Check perks and spells against the engine* asks the engine's own `HasSpell` about every castable spell on each companion's record and every taught spell, and asks `CheckCast` about the set-aside ones. The log gets the disagreements. Our own `HasSpell` calls count as walks too, so the counter that shows the engine using the hook is the combat one. It counts walks whose visitor is the AI's `GatherSpellsFunctor` (CommonLib's `VTABLE_GatherSpellsFunctor`, SE 264486 / AE 210289), and should move when a fight starts with a companion present. The refused-casts counter leaves out the check's own questions.
