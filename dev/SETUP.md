...
[dev/DOWNGRADE.md](DOWNGRADE.md). Run `python tools/check_install.py "<Skyrim folder>"`
before and after.
...
`dev/TESTING.md`. The CK becomes necessary in Phase 4, for the ESP's combat styles,
...
- The four console batch files are copied to the game root (`ftsetup.txt`, `ftspawn.txt`,
  `fthurt.txt`, `ftclean.txt`). See `dev/TESTING.md`.
