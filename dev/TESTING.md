...
All four of the extractions this section once listed as still to come -- the follower lease state machine, the player cast state machine, the bash transitions and snapshot assembly one family at a time -- are done, in the shape `ActorTick` has: a state, a struct of what was read, a plan out, the game calling the engine from the plan. The ledger below names where each landed. What is left is `GAME_MODEL.md` part two, the fake engine and the eleven scenarios end to end, folding in the watchdog simulation of `test_loadout.cpp`.

Two things the review of 2026-09-19 asked for wait on that fake engine and cannot be had before it, because in each the untested part IS the engine translation rather than a decision that can be lifted out: that the spell step of the snapshot emits the entries it should and prices exactly the ids a rule named (the assembly around `ClassifySpells`, which is tested; the gate itself is one `find` over the ids the rules name), and that `ScanInventory` hands every rendered row the count, variant and extra-list token that belong to it (`DisplayRows` decides which rows there are, and is tested; what is unproven is the temporary `InventoryEntryData` each row is described from). Neither is a wrapper away.

Judged not worth extracting yet: the hit sinks' classification (`Hits.cpp` is fifty lines of engine reads over the tested `HitTable`); the poison dose and recharge arithmetic (two lines each, the rest engine calls); the settings owner. The panel's cell readers, the click's next request, the sort and the filters in `game/ui` are free of ImGui calls and could be tested (a review of 2026-09-19 made the point; "ImGui-bound" was too broad), but each is a few lines over row types that live in game headers, so they are low on the list rather than off it. The assessment's eight reverse-engineering questions are research, not tests, and stay where `dev/UNIQUE.md` and `dev/TODO.md` keep them. Its harness advice is followed as written: Catch2 only, no fake `RE::Actor`, no vtable patching, no sleeps; every timer test supplies `now`, every bag is built by hand.

Found by the review of 2026-09-19 and fixed the same day: the tick handed an idle list's leftover sequence to a combat list switched on mid-fight (`core/Tick.h`, now dropped on the handoff, tested through the evaluator); the rules page wrote a renamed copy of the list back whole after building a snapshot, over any edit the panel made meanwhile (now renamed in place under the lock, the rename a tested core function, `RefreshActionNames`); the bag step of the snapshot repeated the spell step's scrolls, spells and soul gems. Its three design notes -- capture an actor's two rule lists once per tick so pricing and evaluation read one version; carry the panel's request as a value with the copy question (`EquipAsk`) rather than an origin flag; the pin watchdog's fight lifecycle as transitions over the books -- are all applied; the ledger below names where.
...
- [x] Follower spell and voice lease: `LeaseState`, `AdvanceCast` (core/Lease.h); the tick reads the engine and the lease's watch into a `LeaseSeen`, acts on the step. Cleanup order stays in `Release`.
- [x] Weapon lease: `AdvanceWeapon`, removed with the power attack package on 2026-09-25. A follower's power attack is `StrikeState`, `AdvanceStrike` (core/Strike.h), the action through a callback the test scripts, as the bash's.
- [x] Player cast: `CastState`, `AdvancePlayerCast` (core/PlayerCast.h), every step, window and reason, the commands through a callback the test records. Still in the game, engine reads through and through: `ChooseHand`, `Lend`, `Restore`, `PlayerHeld`.
...
- [x] The per-actor turn itself (`core/Coordinator.h`: `DecideTurn`, `NoteOutcome`): the finished request's cooldown, the list, the snapshot asked for only when there is one to evaluate, the evaluation, and what came of the action. The follower's turn and the player's are one function in the game (`RunTurn`), and `tests/test_coordinator.cpp` drives that same function rather than an imitation of it. (A first harness reimplemented the glue and left the busy capability out, and reported a refire the game never makes; corrected the same day.)
- [x] The pin watchdog: `FightBook` (core/Watchdog.h) remembers, mirrors the panel's word and settles; `JudgePin` and `JudgeBan` answer for one pin and one ban; `PlanWatch` is the whole pass in order -- the pins put back, then the banned things taken off, a cast of ours suspending the bans, a ban marked as the fight's when a rule's pin on it has just gone. `EnforcePins` reads each thing, asks, reports and performs the steps after the lock, as before.
...
- [x] The animation graph's events: one sink (`game/Graph.cpp`) over `GraphWatches` (core/GraphEvents.h), the tag, what fired, the counting, a request's own fires, the CastStop after one, the wakes; each machine reads its watch through `Hear` and takes its baselines itself, the player's cast included (`interruptsAtLend`, `firesAtPress`). Three sinks with their own flags and locks did this until 2026-09-25. `tests/test_graphevents.cpp` tests the watches' own rules. The switch was checked once against the three old sinks, transcribed as they stood and run beside the watches over random event streams, where the machines decided the same but for the player's hand keeping only its last fire, now counted; the check was taken out once it had passed, since it held the new code to the old one's quirks, and the commit that brought the listener in still carries it. `tests/test_replay.cpp` replays sequences from play through the watches and the machines to the outcomes the log reported, the reads at each step reconstructed; since every blow and lease step logs its reads (`ReadsOf`), a later log replays as it stands, its counts checked against the watch's. Fourteen deliberate breaks of the watches and their readers each fail the tests.
...
- [x] Custom skills loading, all the way down: which files and in what order is `JsonFilesIn` and `ReadText` (core/Files.h), tested against a real folder -- by name, any case of the extension, no folders, a missing folder giving nothing; that one bad neighbour is not the end of the walk is `LoadSkillTrees` (core/CustomSkills.h), a file that will not parse or read and a skill the build throws over each costing only itself; and a perk's ranks are `RankChain`, the one walk both the framework's trees and the vanilla ones use, which stops at a rank it has already seen rather than repeating it to the bound. The parser and the tree order were tested already. What is left in the game is the form lookup and the translation of a skill's name, which is one engine call over "the name, or the id where there is none".
