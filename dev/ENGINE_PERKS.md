...
| An ability entry (Magic Resistance, Recovery) | 23796 / 23797 (`BGSAbilityPerkEntry`, vtable 195297) | | Apply is `Actor::AddSpell`, remove is `Actor::RemoveSpell` (38716 / 38717): the ability goes in the actor's added spells, and what it does to a value is a temporary modifier, **which the save keeps either way** |
...
- **The record is never written, and the view is not saved.** Before a load the views are dropped; after it they are published again from the save's ledger. A bought **ability** perk is the one thing the engine puts on the actor: the entry's apply (23796) is `AddSpell` (38716), which applies the ability and lists the spell, and what the ability does to a value is a temporary modifier. **A save keeps that modifier** -- measured 2026-09-22, and keeping the spell off every list does not change it. So the abilities of perks bought here come off before the engine writes and go back on the moment it is done (`BeforeSave`, progression/game/Service.h): `RemoveSpell` (38717) dispels (`MagicTarget::DispelEffect`, 34505) and unlists, `AddSpell` puts it back, and an SKSE task runs after the write, which the engine does inside the call SKSE's message came from. Every save is written with none of it, so the mod can be removed with no step first. Turning progression off drops them the same way, through the release. An ability of their own, or one their record lists, is left alone: the engine applies it from the record on every load, as it always has.
...
- **Ability perks, applied and not listed, and off before a save.** Read from code: that `AddSpell` applies before it asks the walk, and appends only what the walk does not know (38716), and that `RemoveSpell` dispels first (38717). Seen in play 2026-09-22 (vanilla, Magic Resistance on a follower): the ability's effect arrives with the perk and goes with it. The same day, the save's own answer: **a save keeps the temporary modifier** an ability made, and keeping the spell off every list does not change that. With the plugin gone, the follower still read +10 Magic Resistance, `hasspell` on the ability answered 0, `getbaseav` 0, and a dispel took nothing off -- a modifier with nothing behind it. Hence `BeforeSave` (progression/game/Service.h), seen to work the same day: the save it writes loads clean without the plugin. What is not measured: whether the modifier a save keeps would double if the ability were applied again on top of it; loading with the plugin on read 10, not 20, so the engine does not stack it.
