...
| Implementation | **C++ SKSE plugin** with runtime-created records; no ESP or Papyrus shell |
| UI and persistence | **In-game ImGui** (SKSE Menu Framework); rules and settings in the **SKSE co-save** |
...
The implemented serialization format is documented in `dev/PROFILES.md` and defined by
...
Rules, follower switches, pins and bans are serialized as one JSON document per follower
inside the SKSE co-save. Global settings have their own record. Changes are written when
the game saves and restored when it loads; cooldowns and in-progress actions are transient.
There are no standalone profile files. Shareable profile export remains future work.
See `dev/PROFILES.md` for the schema, follower keys and form resolution.
...
There is no ESP or Papyrus shell. Package and wrapper records are created in memory for
each follower when first needed. See `src/game/Forms.cpp` and `dev/MAGIC.md` ("Forms at
runtime").
...
2. **SKSE64 2.2.8** (the build for 1.6.1170 — *not* the current 2.3.1, which targets
   ...
   this mod). Full step-by-step in `dev/SETUP.md`.
...
6. Keep the C++ source, tests, tools and documentation version-controlled.
...
- JSON serialization in the SKSE co-save; schema and compatibility rules in `dev/PROFILES.md`
...
**Exit criterion:** three rules of different shapes all firing correctly,
...
unsupported. Co-save persistence as documented in `dev/PROFILES.md`. Profile sharing,
cloning and defaults per archetype (tank / healer / archer / mage) remain future work.
