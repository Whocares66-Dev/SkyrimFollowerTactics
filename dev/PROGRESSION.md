...
**Innate perks** — those on the follower's own record, like Marcurio's six — are chosen for them in advance: held, counted against their perk points as the player's own choices are, and theirs to give back like any perk bought here. A right click on the top rank gives it back and returns its point, unless a perk they hold needs it; the record keeps it, and a click takes it up again for a point, whatever it asks, since it was theirs. *Reset perks* gives back the tree's own perks with the bought. So a follower can be rebuilt whole, perk by perk. On the skill page their ring is amber, of a piece with the gold beside their level, and faded while given back, and their name's hover says *Acquired outside of this framework*: what the record gave them, their plugin's or a distributor's, as against what was bought here. This is possible because the engine is told what a companion holds rather than having their record edited ([ENGINE_PERKS.md](ENGINE_PERKS.md)). *(Until 2026-09-22 an innate perk could only be set aside, for nothing, and restored for nothing.)*
