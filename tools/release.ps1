...
param(
    ...
    # Skip the clean-tree and up-to-date checks, for a release cut from a
    # master that is ahead of origin or has edits in flight. It does NOT
...
if (-not $Force) {
    ...
    if ($local -ne $upstream) { Fail "master and origin/master differ. Push or pull first (or pass -Force)." }
}
...
git -C $root ls-remote --exit-code --tags origin $tag *> $null
...
$arguments = @('release', 'create', $tag, '--repo', $repo, '--title', "$name $version", '--generate-notes')
