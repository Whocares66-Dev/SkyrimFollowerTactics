...
NPC_FLAGS = [(0x2, 'BASE_DATA'), (0x4, 'ATTRIBUTES'), (0x8, 'AIDATA'), (0x10, 'SPELLLIST'), (0x20, 'FULLNAME'),
             (0x40, 'FACTIONS'), (0x200, 'SKILLS'), (0x400, 'CLASS'), (0x800, 'FACE'), (0x1000000, 'DEFAULT_OUTFIT'),
             (0x2000000, 'SLEEP_OUTFIT'), (0x4000000, 'GENDER'), (0x8000000, 'RACE')]
...
class Save:
    def __init__(self, folder, prefix):
        ...
        body = lz4_block(raw, ulen) if comp == 2 else (zlib.decompress(raw) if comp == 1 else raw)
    ...
    def owner(self, fid):
        ...
        if top == 0xFE:
            i = (fid >> 12) & 0xFFF
            return self.light[i] if i < len(self.light) else f'LIGHT SLOT {i} NOT IN THIS SAVE'
        return self.plugins[top] if top < len(self.plugins) else f'INDEX {top:02X} NOT IN THIS SAVE'
...
def npc(save, target):
    for ref, flags, typ, ver, l2, data in save.forms:
        ...
        print(f'  {target:08X}: type {typ} flags {flags:08X} ({", ".join(names)}) version {ver}, {len(d)} bytes')
        if flags & (0x4 | 0x20):
            print('  attributes or a full name present: section order not established; hex only')
            print('  hex:', d.hex(' '))
            return
...
def scan(save):
    ...
    for ref, flags, typ, _ver, l2, data in save.forms:
        ...
        if hits:
            found += 1
            print(f'  {save.describe(ref)}: type {typ} flags {flags:08X}, ' +
...
def main():
    ...
    if '--folder' in args:
        ...
        folder = args[i + 1]
        del args[i:i + 2]
    if len(args) < 2 or args[1] not in ('npc', 'scan') or (args[1] == 'npc' and len(args) < 3):
        raise SystemExit(__doc__)
    save = Save(folder, args[0])
    print(f'{os.path.basename(save.path)}: {len(save.plugins)} plugins + {len(save.light)} light, '
