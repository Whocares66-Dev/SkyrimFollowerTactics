...
def main(args):
    ...
    if outer < 0x80:
        pattern = re.compile(rb"[\x48-\x4f]\x8b([\x40-\x7f])" + b"(?:.)?" + re.escape(bytes([outer])), re.S)
    else:
        pattern = re.compile(rb"[\x48-\x4f]\x8b([\x80-\xbf])" + b"(?:.)?" + re.escape(outer.to_bytes(4, "little")), re.S)
    ...
    for m in pattern.finditer(data):
        ...
        if dst.type != x86.X86_OP_REG or src.type != x86.X86_OP_MEM or src.mem.disp != outer:
            continue
        reg = dst.reg
        for ins in insns[1:window]:
            hit = False
            for op in ins.operands:
                if op.type == x86.X86_OP_MEM and op.mem.base == reg and op.mem.index == 0 and op.mem.disp in inner and (
                    size is None or op.size == size
                ):
                    hit = True
            if hit:
            ...
            if reg in written or ins.mnemonic in ("ret", "jmp", "call") and ins.mnemonic != "call":
                break
            if ins.mnemonic == "call" and reg in (x86.X86_REG_RAX, x86.X86_REG_RCX, x86.X86_REG_RDX,
    ...
    for (oid, ooff), hits in sorted(found.items(), key=lambda kv: kv[0][1]):
        ...
        if sites:
            for rva, ins in hits:
                print(f"    load at {rva:#x}, then {ins.address:#x}: {ins.mnemonic} {ins.op_str}")
    print(f"{sum(len(h) for h in found.values())} match(es) in {len(found)} function(s)")
